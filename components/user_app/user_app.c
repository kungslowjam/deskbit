...
void user_Click_Event_Init(lv_ui *ui) {
  lv_obj_add_event_cb(ui->screen_btn_1, screen_btn_event_handler, LV_EVENT_ALL,
                      ui);
  lv_obj_add_event_cb(ui->screen_btn_2, screen_btn_event_handler, LV_EVENT_ALL,
                      ui);
  lv_obj_add_event_cb(ui->screen_imgbtn_1, screen_btn_event_handler,
                      LV_EVENT_ALL, ui);
  lv_obj_add_event_cb(ui->screen_imgbtn_2, screen_btn_event_handler,
                      LV_EVENT_ALL, ui);
  lv_obj_add_event_cb(ui->screen_btn_5, screen_btn_event_handler, LV_EVENT_ALL,
                      ui);
  lv_obj_add_event_cb(ui->screen_btn_6, screen_btn_event_handler, LV_EVENT_ALL,
                      ui);
  lv_obj_add_event_cb(ui->screen_slider_1, screen_btn_event_handler,
                      LV_EVENT_ALL, ui);
}
void user_task_init(lv_ui *ui) {
  ...
  xTaskCreate(example_GPIO_task, "example_GPIO_task", 3000, ui, 2,
              NULL); // gpio Test
  xTaskCreate(example_KEY_task, "example_KEY_task", 3000, ui, 2,
              NULL); // KEY Test
}
void user_Init(lv_ui *ui) {
  ...
  esp32_gpio_init();
  button_Init();
  // nvs_flash_Init(); // Moved to app_main.c (called before ui_settings_init)
  ble_scan_class_init();
  ble_scan_Init();
  // espwifi_Init(); // Moved to app_main.c (called before ui_settings_init)
...
void example_KEY_task(void *arg) {
  ...
  for (;;) {
    EventBits_t even = xEventGroupWaitBits(
        key_groups, (0x01 << 1) | (0x01 << 0), pdTRUE, pdFALSE, 1000);
    if ((even >> 0) & 0x01) {
...
static void clock_task_callback(void *arg) {
  ...
  if (clock_iniput.out_seconds == 60) {
    ...
    if ((clock_iniput.out_minutes == 12) || (clock_iniput.out_minutes == 24) ||
        (clock_iniput.out_minutes == 36) || (clock_iniput.out_minutes == 48) ||
        (clock_iniput.out_minutes == 60))
      bat = 1;
    else
      bat = 0;
  }
...
void SetTheClock_start(lv_ui *ui) {
  ...
  ESP_ERROR_CHECK(
      esp_timer_start_periodic(clock_tick_timer, 1000 * 1000)); // 1s
}
void user_example(void *udata) {
...
static void screen_btn_event_handler(lv_event_t *e) {
  ...
  case LV_EVENT_CLICKED: {
    EventBits_t even = xEventGroupWaitBits(TaskEven, (0x01 << 1) | (0x01 << 2),
                                           pdFALSE, pdFALSE, 10);
...
// OLD WiFi scan function removed - replaced by new Settings UI WiFi system
// See esp_wifi_bsp.c and ui_settings.c for new implementation
  ...
  for (;;) {
    ...
    for (; xQueueReceive(ble_Queue, mac, 3500) == pdTRUE;) {
      ...
      if (imte != NULL) {
        ...
        if (label != NULL) {
          sprintf(imgbox, "%d:%d:%d:%d:%d:%d", mac[0], mac[1], mac[2], mac[3],
                  mac[4], mac[5]);
